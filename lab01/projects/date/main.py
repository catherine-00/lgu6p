...
#   길동이는 파이썬으로 (1)태어난 년, 월, 일을 입력받고 (2)현재 또는 특정 날의 년, 월, 일을 입력받아 
#   (3) 두 시점의 날 수 차이를 계산하는 프로그램을 만들고 싶었다.
#
# - 다음 조건을 만족 시키면서 해당 프로그램을 작성하시오.
#     - 1. 입력은 반드시 숫자로만 주어져야 한다. 
#       반복문을 적절히 사용하여 사용자가 올바른 입력을 할때까지 입력을 받도록 작성
#     - 2. 문제를 간단히 하기 위해 뒤에 입력하는 연도는 앞에 입력하는 연도보다 적어도 1 커야 한다. 
#       즉 동일한 연도에 두 날짜 차이는 계산하지 않는다.
#       (실제로 1년도 살지 않은 아기의 나이를 비교할 일은 없으니까!)
#     - 3. 날 수를 계산할 때 반드시 윤년을 검사해서 적절히 1일 씩 더 해주어야 함. 
...
# Mission 1에서 완성되어야 함.
from days_utils import MONTH_DAYS, is_leap_year, get_month_days #윤년 계산기
...
def my_days(birth_year, birth_month, birth_day, cur_year, cur_month, cur_day):
    # # birth_year: 태어난 해
    # # birth_year = int(input("태어난 년도을 입력하시오 : "))
    # # birth_month: 태어난 달
    # # birth_month = int(input("태어난 월을 입력하시오 : "))
    # # birth_day: 태어난 날
    # birth_day = int(input("태어난 날짜를 입력하시오 : "))
    # # cur_year: 현재 해 또는 차이를 계산하고 싶은 특정 해
    # cur_year = int(input("현재 혹은 계산하고 싶은 년도을 입력하시오 : "))
    # # cur_month: 현재 달 또는 차이를 계산하고 싶은 특정 달
    # cur_month = int(input("현재 혹은 계산하고 싶은 월을 입력하시오 : "))
    # # cur_day: 현재 날 또는 차이를 계산하고 싶은 특정 날
    # cur_day = int(input("현재 혹은 계산하고 싶은 날짜를 입력하시오 : "))
    ...
    if birth_month == 2 and is_leap_year(birth_year) == True: #2월이고 윤년이면
        days_part_1 += 1 #윤년 2월은 산 날+1
    
    # 태어난 해의 태어난 달 다음달 부터 12월 31일까지 날을 계산해서
    # days_part_1에 더함
    # range에 적당한 범위를 설정하시오.
    for m in range(birth_month+1, 13): #5~12
        days_part_1 += get_month_days(birth_year, m) #각 달에 날짜 수를 계산
    

    #####################################################################      
    # step 2: 태어난 해 다음 해(2000+1) 부터 현재 해(2025-1) 이전 해까지 산 날 수를 계산해서 
    #         days_part_2에 저장
    ...
    for y in range(birth_year+1, cur_year): #2001년부터 2024년까지
        # 안쪽 for 루프에서 y 년도의 달을 m으로 순회하면서 
        for m in range(1,13): #1월~12월
            # days_part_2에 y년 m월의 날 수를 합산
            days_part_2 += get_month_days(y, m) #알아서 윤년 검사해 해서 리턴해줌.


    #####################################################################  
      
    # step 3: 현재 해(2025) 산 날 수를 계산해서 days_part_3에 저장
    #
    days_part_3 = 0

    # 1월 부터 현재 달 이전 달까지 산 날수를 모두 계산해서
    # days_part_3에 저장
    for m in range(1, cur_month): #1~2월까지
